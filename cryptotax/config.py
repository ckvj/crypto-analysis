...
class Config:
    ...
    def accounting_type(self):
        if not self._accounting_type:
            self._accounting_type = self.config["accounting_type"]["accounting_type"]
    ...
    def set_csv_filepath(self):
        if self.config["file_info"].get("dir", None) == None:
            ...
            self.csv_filepath = (
                self.config["file_info"]["dir"] + self.config["file_info"]["filename"]
            )
