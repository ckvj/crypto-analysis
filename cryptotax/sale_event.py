...
class SaleEventBuilder:
    ...
    def calc_clip_size(self):
        self.sale_event.clip_size = min(
            self.sale_event.purchase.remaining, self.sale_event.sale.remaining
        )
        return self

    def calc_gain_loss(self):
        self.sale_event.gain_loss = float(self.sale_event.clip_size) * (
            self.sale_event.sale.price - self.sale_event.purchase.price
        )
        return self
    ...
    def is_long_term(self):
        ...
        time_delta = relativedelta.relativedelta(
            self.sale_event.sale.trade_time, self.sale_event.purchase.trade_time
        )
        ...
        self.sale_event.long_term = time_delta.years >= 1
        return self
